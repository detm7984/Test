...
namespace Importar_Data
{
    public partial class Form1 : Form
    {
        ...
        public void GetColumns(string Servidor, string DataBase, string Tabla)
        {
            ...
            string sel = "Use [" + DataBase + "] Select COLUMN_NAME as Campo, DATA_TYPE as Tipo, CHARACTER_MAXIMUM_LENGTH as Tamaño From INFORMATION_SCHEMA.COLUMNS Where TABLE_NAME ='" + @Tabla + "'";
        ...
        private void cmdAnalizis_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (TxtPath.Text.Length > 0 && TablaA.Rows.Count > 0 && Lst_Campos_Excel.Items.Count > 0)
                {
                    ...
                    foreach (DataColumn myProperty in TablaA.Columns)
                    {
                        nTamaño = nFlagNum = nFlagEntero = 0;
                        ...
                        foreach (DataRow row in TablaA.Rows)
                        {
                            if (row.RowState != DataRowState.Deleted)
                            {
                                ...
                                if (nTamaño < row[myProperty.ColumnName].ToString().Trim().Length)
                                    nTamaño = row[myProperty.ColumnName].ToString().Trim().Length;
                        ...
                        if (Es_Numero != true)
                        {
                            if (nTamaño >= 0 && nTamaño <= 4000)
                            {
                                //Establecer Valor Por Defecto Si El Tamaño Es 0
                                if (nTamaño == 0)
                                    nTamaño = 10;
                                sTipo = "VarChar(" + nTamaño + ")";
                            }
                            if (nTamaño > 4000)
                                sTipo = "Text";
                        }
